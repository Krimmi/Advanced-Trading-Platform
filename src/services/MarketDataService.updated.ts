...
import { API_KEYS, useRealData, DATA_SOURCE_CONFIG } from '../config/apiConfig';
...
export default class MarketDataService {
  ...
  public async getMarketBreadth(index?: string): Promise<MarketBreadthData> {
    ...
    if (useRealData('alphaVantage')) {
      try {
        // Check cache first
        const cacheKey = `market-breadth-${index || 'all'}`;
        const cachedData = this.getFromCache(cacheKey, 'marketData');
        if (cachedData) {
          return cachedData;
        }
        ...
        params.append('apikey', API_KEYS.alphaVantage as string);

        const response = await axios.get(`${this.apiUrl}/api/market-data/breadth?${params.toString()}`);
        
        // Cache the response
        this.addToCache(cacheKey, response.data, 'marketData');
        ...
        if (DATA_SOURCE_CONFIG.useMockDataAsFallback) {
          console.log('Falling back to mock market breadth data');
      ...
      // Use mock data
      console.log('Using mock market breadth data (API key not configured)');
      return this.getMockMarketBreadthData();
    }
  }
  ...
  public async getAnomalyDetection(category?: string, severity?: string): Promise<AnomalyData> {
    // Check if we should use real data
    if (useRealData('polygon')) {
      try {
        // Check cache first
        const cacheKey = `anomalies-${category || 'all'}-${severity || 'all'}`;
        const cachedData = this.getFromCache(cacheKey, 'marketData');
        if (cachedData) {
          return cachedData;
        }
        ...
        // Add API key to request
        params.append('apikey', API_KEYS.polygon as string);
        ...
        // Fall back to mock data if configured
        if (DATA_SOURCE_CONFIG.useMockDataAsFallback) {
          console.log('Falling back to mock anomaly data');
      ...
      // Use mock data
      console.log('Using mock anomaly data (API key not configured)');
      return this.getMockAnomalyData();
    }
  }
  ...
  public async getCorrelationMatrix(assetClass?: string, timeframe?: string): Promise<CorrelationData> {
    // Check if we should use real data
    if (useRealData('iexCloud')) {
      try {
        // Check cache first
        const cacheKey = `correlations-${assetClass || 'all'}-${timeframe || 'default'}`;
        const cachedData = this.getFromCache(cacheKey, 'marketData');
        if (cachedData) {
          return cachedData;
        }
        ...
        // Add API key to request
        params.append('token', API_KEYS.iexCloud as string);
      ...
      // Use mock data
      console.log('Using mock correlation data (API key not configured)');
      return this.getMockCorrelationData();
    }
  }
  ...
  public async getOrderFlow(symbol?: string, orderType?: string): Promise<OrderFlowData> {
    // Check if we should use real data (using Alpaca for order flow)
    if (useRealData('alpaca')) {
      try {
        // Check cache first
        const cacheKey = `order-flow-${symbol || 'all'}-${orderType || 'all'}`;
        const cachedData = this.getFromCache(cacheKey, 'marketData');
        if (cachedData) {
          return cachedData;
        }
        ...
        const headers = {
          'APCA-API-KEY-ID': (API_KEYS.alpaca?.apiKey as string),
          'APCA-API-SECRET-KEY': (API_KEYS.alpaca?.apiSecret as string)
        };

        const response = await axios.get(
          `${this.apiUrl}/api/market-data/order-flow?${params.toString()}`,
          { headers }
        );
        ...
        // Fall back to mock data if configured
        if (DATA_SOURCE_CONFIG.useMockDataAsFallback) {
          console.log('Falling back to mock order flow data');
      ...
      // Use mock data
      console.log('Using mock order flow data (API key not configured)');
      return this.getMockOrderFlowData();
    }
  }
  ...
  public async getRealTimePrices(symbols: string[]): Promise<any> {
    ...
    if (useRealData('alphaVantage')) {
      try {
        // Check cache first (short TTL for real-time prices)
        const symbolsKey = symbols.sort().join(',');
        const cacheKey = `prices-${symbolsKey}`;
        const cachedData = this.getFromCache(cacheKey, 'marketData', 30000); // 30 seconds TTL for prices
        if (cachedData) {
          return cachedData;
        }
        ...
        const response = await axios.post(
          `${this.apiUrl}/api/market-data/prices?${params.toString()}`, 
          { symbols }
        );
        
        // Cache the response
        this.addToCache(cacheKey, response.data, 'marketData', 30000);
      ...
      // Use mock data
      console.log('Using mock price data (API key not configured)');
      return this.getMockPriceData(symbols);
    }
  }
  ...
  public subscribeToMarketData(dataType: string, callback: (data: any) => void): string {
    // In a real implementation with API keys, this would set up a WebSocket connection
  ...
  private addToCache(key: string, data: any, cacheType: keyof typeof DATA_SOURCE_CONFIG.cacheTTL, customTTL?: number): void {
    if (!DATA_SOURCE_CONFIG.enableApiCache) return;
  ...
  private getFromCache(key: string, cacheType: keyof typeof DATA_SOURCE_CONFIG.cacheTTL, customTTL?: number): any {
    if (!DATA_SOURCE_CONFIG.enableApiCache) return null;
    
    const cached = this.cache.get(key);
    if (!cached) return null;
    
    const ttl = customTTL || DATA_SOURCE_CONFIG.cacheTTL[cacheType];
    const now = Date.now();
  ...
  private getMockPriceData(symbols: string[]): any {
  ...
  // The rest of the mock data methods remain the same as in the original file
  // ...
  ...
  private getMockCorrelationData(): CorrelationData {
    // Implementation remains the same as in the original file
    // (This is a large method, so I'm not including it here for brevity)
    const now = new Date();
    const dateStr = now.toISOString();
  ...
  private getMockOrderFlowData(): OrderFlowData {
    ...
    const marketDepth: any[] = symbols.map(symbol => {
      ...
      // Generate ask levels
      const askLevels: any[] = Array(10).fill(0).map((_, i) => {
