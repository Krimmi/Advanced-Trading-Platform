...
// Import virtualized table components for better performance
...
const FeatureImportanceChart = lazy(() => import('./charts/FeatureImportanceChart'));
const ModelPerformanceChart = lazy(() => import('./charts/ModelPerformanceChart'));
const ModelVersionHistory = lazy(() => import('./ModelVersionHistory'));
...
const MLModelManagementPanel: React.FC<MLModelManagementPanelProps> = ({
  ...
  // Fetch models from the API - use useCallback to prevent recreation
  ...
  // Handle search query change - debounced to prevent excessive filtering
  ...
  const handleModelTypeFilterChange = useCallback((event: React.ChangeEvent<{ value: unknown }>) => {
  ...
  }, [mlService, newModelName, newModelType, newModelDescription, onModelCreate]);
  
  // Handle model deletion dialog open
  const handleDeleteDialogOpen = useCallback((model: MLModel) => {
    setSelectedModel(model);
  ...
  }, [mlService, selectedModel]);
  
  // Handle model deployment dialog open
  const handleDeployDialogOpen = useCallback((model: MLModel) => {
    setSelectedModel(model);
  ...
  }, [mlService, selectedModel, onModelDeploy]);
  
  // Filter and sort models - memoized to prevent recalculation on every render
  ...
  // Paginate models - memoized to prevent recalculation on every render
  ...
  // Get status color - memoized to prevent recalculation
  ...
  // Get status icon - memoized to prevent recreation
  const getStatusIcon = useCallback((status: ModelStatus) => {
    switch (status) {
      case ModelStatus.DEPLOYED:
  ...
  // Format date - memoized to prevent recalculation
  const formatDate = useCallback((dateString: string) => {
    const date = new Date(dateString);
    return date.toLocaleDateString() + ' ' + date.toLocaleTimeString();
  }, []);
  
  // Virtualized row renderer for better performance with large lists
  const ModelTableRow = useCallback(({ index, style }: { index: number; style: React.CSSProperties }) => {
    ...
    return (
      ...
      >
        <TableCell style={{ flex: 3, display: 'flex', flexDirection: 'column' }}>
  ...
  }, [paginatedModels, selectedModel, theme, handleModelSelect, getStatusIcon, formatDate, handleDeployDialogOpen, handleDeleteDialogOpen]);
  ...
  const TableHeader = useMemo(() => (
    <TableRow style={{ display: 'flex', alignItems: 'center' }}>
      <TableCell 
        ...
        sx={{ cursor: 'pointer', fontWeight: 'bold', flex: 3 }}
      >
        Name
        {sortBy === 'name' && (
      ...
      <TableCell 
        ...
        sx={{ cursor: 'pointer', fontWeight: 'bold', flex: 1, textAlign: 'right' }}
      >
        Accuracy
        {sortBy === 'accuracy' && (
          <Box component="span" sx={{ ml: 0.5 }}>
      ...
      <TableCell 
        ...
        sx={{ cursor: 'pointer', fontWeight: 'bold', flex: 2 }}
      >
        Last Updated
        {sortBy === 'lastUpdated' && (
          <Box component="span" sx={{ ml: 0.5 }}>
      ...
      <TableCell sx={{ fontWeight: 'bold', flex: 1, textAlign: 'right' }}>
        Actions
      </TableCell>
    </TableRow>
  ), [sortBy, sortDirection, handleSortChange]);
  
  // Render model list with virtualization for better performance
  const renderModelList = useMemo(() => {
    ...
    searchQuery, modelTypeFilter, statusFilter, handleSearchChange, handleModelTypeFilterChange, 
    handleStatusFilterChange, handleCreateDialogOpen, TableHeader, loading, models.length, 
    paginatedModels, filteredModels.length, rowsPerPage, page, handleChangePage, 
    handleChangeRowsPerPage, ModelTableRow
  ]);
  
  // Memoized model details component
  const renderModelDetails = useMemo(() => {
    if (!selectedModel) {
      return (
        <Box sx={{ 
          height: '100%', 
    ...
    selectedModel, getStatusIcon, formatDate, activeTab, handleTabChange, 
    theme, handleDeployDialogOpen
  ]);
  
  // Create model dialog - memoized to prevent recreation
  const createModelDialog = useMemo(() => (
    ...
    createDialogOpen, handleCreateDialogClose, newModelName, newModelType, 
    newModelDescription, handleCreateModel
  ]);
  
  // Delete model dialog - memoized to prevent recreation
  const deleteModelDialog = useMemo(() => (
  ...
  ), [deleteDialogOpen, handleDeleteDialogClose, selectedModel, handleDeleteModel]);
  
  // Deploy model dialog - memoized to prevent recreation
  const deployModelDialog = useMemo(() => (
  ...
  ), [deployDialogOpen, handleDeployDialogClose, selectedModel, handleDeployModel]);
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box>
          <IconButton onClick={fetchModels} sx={{ mr: 1 }} disabled={loading}>
...
export default React.memo(MLModelManagementPanel);